import { Avatar, Checkbox, Select } from "@chakra-ui/react";
import { useFormContext } from "react-hook-form";
import { useState } from "react";

const animation = 'transition-all ease-out duration-200'

let avatatUrl = 'data:image/png;base64,'

export const AvatarWithUser = ({ set, user }: any) => {
    const [isChecked, setIsChecked] = useState<boolean>(false);
    const { setValue, getValues } = useFormContext();

    const handleAddMember = (e: any) => {
        const currentMembers = getValues('members') || [];
        if (isChecked) {
            const selectedMembers = currentMembers.filter(
                (member: any) => member._id !== user._id)
            console.log({ selectedMembers });
            setValue('members', selectedMembers);
            setIsChecked(!isChecked);
            set((prev: boolean) => !prev);
        } else {
            setValue('members', [...currentMembers, user]);
            setIsChecked(!isChecked);
            set((prev: boolean) => !prev);
        }
    };
    const handleRoles = (e: any) => {
        console.log(e.target.value === 'Admin')
        switch (e.target.value) {
            case 'Admin':
                handleAdmin();
                break;
            case 'Manager':
                handleManager();
                break;
            case 'Member':
                handleMember();
                break;
        }
    };
    function isManager(userId: string): boolean {
        return (getValues('manager') || []).includes(userId);
    };
    function isAdmin(userId: string): boolean {
        return (getValues('admins') || []).includes(userId);
    };
    function removeFromAdmin(userId: string): void {
        setValue(
            'admins',
            getValues('admins').filter((m: any) => m._id !== userId)
        )
    };
    function removeFromManager(userId: string): void {
        setValue(
            'managers',
            getValues('managers').filter((m: any) => m._id !== userId)
        )
    };
    function handleAdmin() {
        if (isManager(user?._id)) {
            removeFromManager(user._id)
        }
        const currentAdmins = getValues('admins') || [];
        setValue('admins', [...currentAdmins, user]);
    };
    function handleManager() {
        console.log('called manager');
        if (isAdmin(user?._id)) {
            removeFromAdmin(user._id)
        }
        const currentManagers = getValues('managers') || [];
        setValue('managers', [...currentManagers, user]);
    };
    function handleMember() {
        if (isManager(user._id)) removeFromManager(user._id);

        if (isAdmin(user._id)) removeFromAdmin(user._id);
    };
    return (
        <div className={`${animation} flex  hover:bg-slate-50 hover:cursor-pointer px-3 my-1 items-center justify-between`}>
            <div className={` ${animation} flex px-3 my-3 items-center justify-start space-x-4`}>
                <Checkbox onChange={handleAddMember} size='md' colorScheme='blue' isChecked={isChecked} />
                <Avatar
                    color={'white'}
                    bgColor={'orange'}
                    colorScheme="red"
                    size={'md'}
                    name='Kola Tioluwani'
                    src={avatatUrl}
                />
                <div>
                    <p className="font-lato text-gray-800">
                        {user.name}
                    </p>
                    <p className="text-sm text-gray-600 font-lato">
                        Student
                    </p>
                </div>
            </div>
            <Select onChange={handleRoles} className="text-xs" maxWidth={'fit-content'} defaultValue={'Member'}>
                <option value='Admin'>Admin</option>
                <option value='Manager'>Manager</option>
                <option value='Member'>Member</option>
            </Select>
        </div>
    )
}